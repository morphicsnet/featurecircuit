...
└─ runs/                  # Stamped artifacts per run
```

## Rust Core (nsi_core)

### Encoders and ensembles
- `core/nsi_core/src/ensemble.rs`
  - `pub trait Encoder`
...
- `core/nsi_core/src/hypergraph.rs`
...
Bindings in `core/py_nsi/src/lib.rs` expose a minimal, deterministic surface:
...
- Config-first reproducibility: each demo writes its YAML to `runs/<demo>/<run_tag>/config.yaml`.
