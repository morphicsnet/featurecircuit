...
This document defines the YAML schemas used by the demo scripts in `python/`. Each demo reads a single YAML file from `configs/` and writes a frozen copy to `runs/<demo>/<run_tag>/config.yaml`.
...
- `outputs.base_dir`: default `runs/ensemble`
...
- `outputs.base_dir`: default `runs/spike_hypergraph`
...
- `acdc` (mapping):
  ...
  - `max_edges` (int)
- `outputs.base_dir`: default `runs/causal`
