...
Per run directory under `runs/<demo>/<run_tag>/`:
...
- Reproducibility
  - All plots and JSON metrics are written directly into `runs/<demo>/<run_tag>/` alongside `config.yaml` (see [REPRODUCIBILITY.md](REPRODUCIBILITY.md)).
...
All thresholds, seeds, and configuration values must be recorded in the run’s `runs/<demo>/<run_tag>/config.yaml`, ensuring full reproducibility across environments.
