...
This runbook covers deterministic builds, demo execution, and artifact packaging. Demo scripts write a frozen copy of the active config to `config.yaml` inside each run directory. Artifacts are grouped by demo under `runs/<demo>/<run_tag>/`.
...
## Build Steps (py_nsi)
The Python bindings in `core/py_nsi/` wrap the Rust core `core/nsi_core/`.
...
- Demo 5 reads artifacts from prior runs and writes a summary to `runs/investor/<run_tag>/`.
...
run_dirs = {
    "baseline": "runs/baseline/<run_tag>",
    "ensemble": "runs/ensemble/<run_tag>",
    "spike_hypergraph": "runs/spike_hypergraph/<run_tag>",
    "causal": "runs/causal/<run_tag>",
}
artifacts = collect_artifacts(run_dirs)
write_manifest(artifacts, "runs/bundle/manifest.json")
make_zip(artifacts, "runs/bundle/bundle.zip")
```
