...
def _edge_nodes_from_item(item: Any) -> Tuple[List[str], Optional[float], Optional[float]]:
    """
    ...
    if isinstance(item, dict):
        ...
        for k in ("nodes", "members", "incidences", "edge", "node_ids"):
            if k in item and isinstance(item[k], (list, tuple)):
                nodes = [_normalize_node_id(x) for x in item[k]]
                break
        ...
        nodes = [_normalize_node_id(x) for x in item]
    else:
...
def _collect_edges(hif: dict) -> List[Tuple[List[str], Optional[float], Optional[float]]]:
    """
    ...
    edges: List[Tuple[List[str], Optional[float], Optional[float]]] = []
    ...
    seen: set = set()
    deduped: List[Tuple[List[str], Optional[float], Optional[float]]] = []
