...
def main(config_path: str = "configs/demo2_ensemble.yaml") -> None:
    ...
    write_feature_space(
        run_dir,
        {
            ...
            "schema_version": 1,
            "feature_space_id": "demo2.ensemble_intersection",
            "feature_space_type": "ensemble_sae",
            "producer": "demo2_ensemble",
            "producer_version": "v1",
            "model_id": model_name,
            "layer_map": {str(layer_index): "intersection"},
            "dim": int(features_intersection.shape[1]),
            "activation_rule": "intersect_mask",
            "checksum": f"{input_dim}:{feature_dim}:{top_k_ens}:{','.join(map(str, seeds_ens))}",
            "metadata": {"intersect_threshold": intersect_threshold},
        },
    )
...
if __name__ == "__main__":
    main(sys.argv[1] if len(sys.argv) > 1 else "configs/demo2_ensemble.yaml")
