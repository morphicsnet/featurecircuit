...
impl SpikeEncoder {
    ...
    #[staticmethod]
    pub fn from_params(min_val: f32, max_val: f32, t_min: f32, t_max: f32, epsilon: f32) -> Self {
        Self {
            inner: RsSpikeEncoder::new(SpikeEncoderConfig {
                min_val,
                max_val,
                t_min,
                t_max,
...
impl EnsembleEncoder {
    ...
    #[staticmethod]
    pub fn from_seeds(seeds: Vec<u64>, dim: usize, sparsity: f32, agree_threshold: usize) -> Self {
        Self {
            inner: RsEnsembleEncoder::from_seeds(&seeds, dim, sparsity, agree_threshold),
        }
    }
    ...
    pub fn masks_by_encoder(&self, x: Vec<f32>, thresh: f32) -> Vec<Vec<bool>> {
        self.inner.masks_by_encoder(&x, thresh)
    }
}
